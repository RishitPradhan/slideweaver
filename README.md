<p align="center">
  <img src="readme_assets/images/slideweaver-logo.png" height="90" alt="Slideweaver Logo" />
</p>
...
<p align="center">
  ...
  &nbsp;
  <a href="https://x.com/slideweaverai">
...
**Slideweaver** is an open-source application for generating presentations with AI — all running locally on your device. Stay in control of your data and privacy while using models like OpenAI and Gemini, or use your own hosted models through Ollama.
...
> For enterprise use, custom deployments, or partnership opportunities, contact us at **[suraj@slideweaver.ai](mailto:suraj@slideweaver.ai)**.

> [!IMPORTANT]
> Like Slideweaver? A ⭐ star shows your support and encourages us to keep building!

> [!TIP]
> For detailed setup guides, API documentation, and advanced configuration options, visit our **[Official Documentation](https://docs.slideweaver.ai)**

## ✨ More Freedom with AI Presentations

Slideweaver gives you complete control over your AI presentation workflow. Choose your models, customize your experience, and keep your data private.
...
- ✅ **Electron Desktop App** — Run Slideweaver as a native desktop application on Windows, macOS, and Linux (no browser required)
...
<a href="https://slideweaver.ai" target="_blank" align="center">
  
  <img src="readme_assets/cloud-banner.png" height="350" alt="Slideweaver Logo" />
</a>

## Running Slideweaver

You can run Slideweaver in two ways: **Docker** for a one-command setup without installing a local dev stack, or the **Electron desktop app** for a native app experience (ideal for development or offline use).
...
```bash
docker run -it --name slideweaver -p 5000:80  -v "./app_data:/app_data" ghcr.io/slideweaver/slideweaver:latest
```

##### Windows (PowerShell):

```bash
docker run -it --name slideweaver -p 5000:80 -v "${PWD}\app_data:/app_data" ghcr.io/slideweaver/slideweaver:latest
```

#### 2. Open Slideweaver

Open http://localhost:5000 on browser of your choice to use Slideweaver.

> **Note: You can replace 5000 with any other port number of your choice to run Slideweaver on a different port number.**
...
```bash
docker run -it --name slideweaver -p 5000:80 -p 1455:1455 -v "./app_data:/app_data" ghcr.io/slideweaver/slideweaver:latest
```

##### Windows (PowerShell):

```bash
docker run -it --name slideweaver -p 5000:80 -p 1455:1455 -v "${PWD}\app_data:/app_data" ghcr.io/slideweaver/slideweaver:latest
```

### Option 2: Electron (Desktop App)

Run Slideweaver as a native desktop application. LLM and image provider (API keys, etc.) can be configured in the app; the same environment variables used for Docker apply when running the bundled backend.
...
```bash
docker run -it --name slideweaver -p 5000:80 -e LLM="openai" -e OPENAI_API_KEY="******" -e IMAGE_PROVIDER="dall-e-3" -e CAN_CHANGE_KEYS="false" -v "./app_data:/app_data" ghcr.io/slideweaver/slideweaver:latest
```

### Using Google

```bash
docker run -it --name slideweaver -p 5000:80 -e LLM="google" -e GOOGLE_API_KEY="******" -e IMAGE_PROVIDER="gemini_flash" -e CAN_CHANGE_KEYS="false" -v "./app_data:/app_data" ghcr.io/slideweaver/slideweaver:latest
```

### Using Ollama

```bash
docker run -it --name slideweaver -p 5000:80 -e LLM="ollama" -e OLLAMA_MODEL="llama3.2:3b" -e IMAGE_PROVIDER="pexels" -e PEXELS_API_KEY="*******" -e CAN_CHANGE_KEYS="false" -v "./app_data:/app_data" ghcr.io/slideweaver/slideweaver:latest
```

### Using Anthropic

```bash
docker run -it --name slideweaver -p 5000:80 -e LLM="anthropic" -e ANTHROPIC_API_KEY="******" -e IMAGE_PROVIDER="pexels" -e PEXELS_API_KEY="******" -e CAN_CHANGE_KEYS="false" -v "./app_data:/app_data" ghcr.io/slideweaver/slideweaver:latest
```

### Using OpenAI Compatible API

```bash
docker run -it -p 5000:80 -e CAN_CHANGE_KEYS="false"  -e LLM="custom" -e CUSTOM_LLM_URL="http://*****" -e CUSTOM_LLM_API_KEY="*****" -e CUSTOM_MODEL="llama3.2:3b" -e IMAGE_PROVIDER="pexels" -e  PEXELS_API_KEY="********" -v "./app_data:/app_data" ghcr.io/slideweaver/slideweaver:latest
```

#### Running Slideweaver with GPU Support
...
Once the NVIDIA Container Toolkit is installed and configured, you can run Slideweaver with GPU support by adding the `--gpus=all` flag:

```bash
docker run -it --name slideweaver --gpus=all -p 5000:80 -e LLM="ollama" -e OLLAMA_MODEL="llama3.2:3b" -e IMAGE_PROVIDER="pexels" -e PEXELS_API_KEY="*******" -e CAN_CHANGE_KEYS="false" -v "./app_data:/app_data" ghcr.io/slideweaver/slideweaver:latest
```
...
For detailed info checkout [API documentation](https://docs.slideweaver.ai/using-slideweaver-api).

### API Tutorials

- [Generate Presentations via API in 5 minutes](https://docs.slideweaver.ai/tutorial/generate-presentation-over-api)
- [Create Presentations from CSV using AI](https://docs.slideweaver.ai/tutorial/generate-presentation-from-csv)
- [Create Data Reports Using AI](https://docs.slideweaver.ai/tutorial/create-data-reports-using-ai)
