...
def remove_length_constraints_from_schema(schema: dict) -> dict:
    """Recursively removes minLength, maxLength, minItems, and maxItems from the schema."""
    result = deepcopy(schema)
    
    def _remove_constraints(node: Any) -> Any:
        if isinstance(node, dict):
            ...
            for key, value in node.items():
                node[key] = _remove_constraints(value)
        elif isinstance(node, list):
            for i, item in enumerate(node):
                node[i] = _remove_constraints(item)
        return node
        
    return _remove_constraints(result)
