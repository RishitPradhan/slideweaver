    ...
    return (
        <div className="space-y-2" data-testid="attachments-uploader">
            <div className="flex items-center justify-between">
                <p className="text-sm text-neutral-300 font-syne">
            ...
            <label
                className={`mt-1 block cursor-pointer rounded-lg border-2 border-dashed px-4 py-6 text-center transition-colors ${isDragging ? 'border-[#dc2626] bg-[#dc2626]/5' : 'border-neutral-800 hover:border-[#dc2626]'}`}
            ...
            >
                ...
                <div className="flex flex-col items-center gap-2">
                    <Paperclip className="h-6 w-6 text-[#dc2626]" />
                    <p className="text-sm font-medium text-gray-800 font-syne">
                        Drag and drop PDF, TXT, PPTX, DOCX, or <span className="text-[#dc2626]">click to browse</span>
                    </p>
                </div>
            </label>
                        ...
                        {filteredFiles.map((file, idx) => (
                            ...
                            >
                                {previewUrls[idx] ? (
                                    ...
                                    <div className="flex h-10 w-10 flex-none items-center justify-center rounded bg-[#222222] text-neutral-300">
                                        <File className="h-5 w-5" />
                                    </div>
                                )}

                                <div className="min-w-0 flex-1">
                                    <p className="truncate text-sm font-medium text-white font-syne" title={file.name}>
                                        {file.name}
                                    </p>
                                    <p className="text-xs text-neutral-400 font-syne">{formatFileSize(file.size)}</p>
                                </div>
