...
const SlideCountSelect: React.FC<{
    ...
    return (
        <Select value={value || ""} onValueChange={onValueChange} name="slides">
            <SelectTrigger
                className="w-[140px]  font-instrument_sans font-medium bg-[#111111] text-neutral-200 hover:bg-[#1a1a1a] focus-visible:ring-[#dc2626]/30 flex items-center gap-2 h-10 rounded-xl px-3 ring-1 ring-inset ring-neutral-800 shadow-sm"
            ...
            <SelectContent className="font-instrument_sans">
                ...
                <div
                    className="sticky top-0 z-10 bg-[#111111]  p-2 border-b"
    ...
    <Popover open={open} onOpenChange={onOpenChange}>
        <PopoverTrigger asChild>
            <Button
                ...
                aria-expanded={open}
                className="w-[180px] flex justify-between items-center gap-2 font-instrument_sans font-semibold overflow-hidden bg-[#111111] text-neutral-200   h-10 rounded-xl px-3 ring-1 ring-inset ring-neutral-800 shadow-sm"
            >
                <span className="flex justify-center items-center gap-2.5">
                    <span className="border border-neutral-800  rounded-md p-1">
        ...
        <div className="flex flex-wrap order-1 gap-4 items-center">
            ...
            <ToolTip content="Advanced settings">
                ...
                <button
                    ...
                    className="ml-auto flex items-center gap-2 text-sm bg-[#111111] text-neutral-200 hover:bg-[#1a1a1a] focus-visible:ring-[#dc2626]/30 h-10 rounded-xl px-3 ring-1 ring-inset ring-neutral-800 shadow-sm font-instrument_sans font-medium"
                ...
                <DialogContent className="max-w-2xl font-instrument_sans">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div className="w-full flex flex-col gap-2">
                            <label className="text-sm font-semibold text-neutral-200">Tone</label>
                            <p className="text-xs text-neutral-400">Controls the writing style (e.g., casual, professional, funny).</p>
                            <Select
                                value={advancedDraft.tone}
                                onValueChange={(value) => setAdvancedDraft((prev) => ({ ...prev, tone: value as ToneType }))}
                            >
                                <SelectTrigger className="w-full font-instrument_sans capitalize font-medium bg-[#111111] border-neutral-700 hover:bg-[#1a1a1a] focus-visible:ring-neutral-700">
                        ...
                        {/* Verbosity */}
                        <div className="w-full flex flex-col gap-2">
                            <label className="text-sm font-semibold text-neutral-200">Verbosity</label>
                            <p className="text-xs text-neutral-400">Controls how detailed slide descriptions are: concise, standard, or text-heavy.</p>
                            <Select
                        ...
                        {/* Toggles */}
                        <div className="w-full flex flex-col gap-2 p-3 rounded-md bg-[#1a1a1a] border-neutral-800">
                            <div className="flex items-center justify-between">
                                <label className="text-sm font-semibold text-neutral-200">Include table of contents</label>
                                <Switch
                            ...
                            <p className="text-xs text-neutral-300">Add an index slide summarizing sections (requires 3+ slides).</p>
                        </div>
                        <div className="w-full flex flex-col gap-2 p-3 rounded-md bg-[#1a1a1a] border-neutral-800">
                            <div className="flex items-center justify-between">
                                <label className="text-sm font-semibold text-neutral-200">Title slide</label>
                                <Switch
                                    checked={advancedDraft.includeTitleSlide}
                                    onCheckedChange={(checked) => setAdvancedDraft((prev) => ({ ...prev, includeTitleSlide: checked }))}
                                />
                            </div>
                            <p className="text-xs text-neutral-300">Include a title slide as the first slide.</p>
                        </div>
                        <div className="w-full flex flex-col gap-2 p-3 rounded-md bg-[#1a1a1a] border-neutral-800">
                            <div className="flex items-center justify-between">
                                <label className="text-sm font-semibold text-neutral-200">Web search</label>
                                <Switch
                                    checked={advancedDraft.webSearch}
                                    onCheckedChange={(checked) => setAdvancedDraft((prev) => ({ ...prev, webSearch: checked }))}
                                />
                            </div>
                            <p className="text-xs text-neutral-300">Allow the model to consult the web for fresher facts.</p>
                        </div>

                        {/* Instructions */}
                        <div className="w-full sm:col-span-2 flex flex-col gap-2">
                            <label className="text-sm font-semibold text-neutral-200">Instructions</label>
                            <p className="text-xs text-neutral-400">Optional guidance for the AI. These override defaults except format constraints.</p>
                            <Textarea
                                ...
                                className="py-2 px-3 border-2 font-medium text-sm min-h-[100px] max-h-[200px] border-red-900/50 focus-visible:ring-offset-0 focus-visible:ring-red-900/50"
                            />
                        </div>
                    </div>
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={handleSaveAdvanced} className="bg-[#dc2626] text-white hover:bg-[#dc2626]/90">Save</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </div>
