...
export const ThemeCard: React.FC<ThemeCardProps> = ({ theme, onSelect, onDelete, showDeleteButton = true }) => {
  ...
  return (<div
    className={` group rounded-xl border w-[305px] cursor-pointer transition-all relative bg-[#111111] border-[#EDEEEF]   hover:shadow-md`}
    onClick={() => onSelect(theme)}

  >
    {showDeleteButton && <button
      className="absolute hidden group-hover:block duration-300 transition-all -top-3 -right-3 z-10 bg-[#111111] rounded-full p-2  border border-[#EDEEEF] hover:bg-[#222222] hover:text-neutral-300"
    ...
    {showDeleteDialog && (
      ...
      >
        ...
        <div
          className="relative bg-[#111111] rounded-2xl w-[340px] shadow-2xl animate-[scaleIn_200ms_ease-out] "
        ...
        >
          <div className="p-6 pb-4 flex flex-col items-center text-center">
            ...
            <p className="text-sm text-neutral-500 leading-relaxed">
              You're about to delete <span className="font-medium text-neutral-300">"{theme.name}"</span>. This action cannot be undone.
            </p>
          </div>
          <div className="flex border-t border-gray-100">
            <button
              onClick={() => setShowDeleteDialog(false)}
              className="flex-1 px-4 py-3.5 text-sm font-medium text-neutral-400 hover:bg-[#1a1a1a] transition-colors"
            >
              Cancel
            </button>
            <button
    ...
    <div className='px-5 border-t rounded-b-xl border-[#EDEEEF] w-full py-2.5 h-[80px] bg-[#111111] flex items-center justify-between'>
      <div>
      ...
      <button
        ...
        className={copied ? "text-green-500" : "text-neutral-500 hover:text-neutral-300"}
