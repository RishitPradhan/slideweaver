...
export const PresentationGrid = ({
  ...
  const ShimmerCard = () => (
    <div className="flex flex-col gap-4 min-h-[200px] bg-[#111111]/70 rounded-lg p-4 animate-pulse">
  ...
  const CreateNewCard = () => (
    <div
      ...
      className="flex flex-col cursor-pointer group ring-1 ring-inset ring-neutral-800 hover:ring-[#8A7DFF]/40 bg-[#111111]/80 rounded-xl overflow-hidden transition-all duration-300 font-syne"
    >
  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 px-6 mt-10 md:grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-6 w-full">
        <div className="flex flex-col gap-4 min-h-[200px] cursor-pointer group ring-1 ring-inset ring-neutral-800 bg-[#111111]/80 rounded-xl items-center justify-center animate-pulse">
          <div className="rounded-full bg-neutral-800 p-4">
            <div className="w-8 h-8" />
          </div>
          <div className="text-center space-y-2">
            <div className="h-4 bg-neutral-800 rounded w-32 mx-auto"></div>
            <div className="h-3 bg-neutral-800 rounded w-48 mx-auto"></div>
          </div>
        </div>
        {[...Array(15)].map((_, i) => (
          <ShimmerCard key={i} />
