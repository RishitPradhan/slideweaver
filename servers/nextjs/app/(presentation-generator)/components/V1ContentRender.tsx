...
export const V1ContentRender = ({ slide, isEditMode, theme }: { slide: any, isEditMode: boolean, theme?: any, enableEditMode?: boolean }) => {
    ...
    const Layout = useMemo(() => {
        console.log("V1ContentRender - Resolving Layout for slide:", slide.layout, "group:", slide.layout_group, "isCustom:", isCustomTemplate);
        if (isCustomTemplate) {
            if (customTemplate) {
                ...
                console.log("V1ContentRender - Compiled Custom Layout:", !!compiledLayout);
                return compiledLayout?.component ?? null;
            }
            console.log("V1ContentRender - customTemplate is null");
            return null;
        } else {
            const template = getLayoutByLayoutId(slide.layout);
            console.log("V1ContentRender - Builtin template resolved:", !!template, "for id:", slide.layout);
    ...
    if (isCustomTemplate && customLoading) {
        return (
            <div className="flex flex-col items-center justify-center aspect-video h-full bg-[#222222] rounded-lg">
                <Loader2 className="w-4 h-4 animate-spin" />
            </div>
        );
    }
    console.log("V1ContentRender - Layout found:", !!Layout, "isEditMode:", isEditMode);


    if (!Layout) {
        if (Object.keys(slide.content).length === 0) {
            return (
                <div className="flex flex-col items-center cursor-pointer justify-center aspect-video h-full bg-[#222222] rounded-lg">
                    <p className="text-neutral-400 text-center text-base">Blank Slide</p>
                    <p className="text-neutral-400 text-center text-sm">This slide is empty. Please add content to it using the edit button.</p>
                </div>
            )
        }
        return (
