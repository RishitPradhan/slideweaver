...
const ThemeSelector = ({ presentation_id, current_theme, themes: allThemes }: { presentation_id: string, current_theme: any, themes: any[] }) => {
    ...
    return (
        <Popover open={isOpen} onOpenChange={setIsOpen}>
            <PopoverTrigger>
                <button className={`text-sm px-[18px] py-2.5 gap-1.5 flex items-center  border border-[#EDEEEF] bg-[#F6F6F9]   duration-300 rounded-[88px] font-medium font-syne ${isOpen ? 'text-[#007AFF]' : 'text-white'}`}>
                    <Palette className={`h-4 w-4 ${isOpen ? 'text-[#007AFF]' : 'text-white'}`} /> Theme
                </button>
            </PopoverTrigger>
            <PopoverContent className="w-fit rounded-[18px] max-h-80 overflow-y-auto hide-scrollbar">
                <div className='pb-2 flex  gap-2 justify-end'>
                    <button className='text-xs text-neutral-500 pb-2 text-right underline' onClick={() => router.push(`/theme?tab=new-theme`)}>+Customize Theme</button>
                    <button className='text-xs text-neutral-500 pb-2 text-right underline' onClick={resetTheme}>Reset Theme</button>
                </div>
                <div className="grid grid-cols-3 gap-4">
                    ...
                    {allThemes && allThemes.length > 0 && allThemes.map((t) => (
                        ...
                        >
                            ...
                            <div className={`rounded-xl cursor-pointer p-1 border shadow-sm bg-[#111111]  transition-all group-hover:shadow-md ${currentTheme.id === t.id ? 'border-red-600 bg-blue-50' : 'border-neutral-800 hover:border-gray-300'}`}>
                            ...
                            <p className="mt-2 text-xs text-center font-medium text-neutral-300 truncate w-full">
                                {t.name}
                            </p>
                        </div>
                    ))}
