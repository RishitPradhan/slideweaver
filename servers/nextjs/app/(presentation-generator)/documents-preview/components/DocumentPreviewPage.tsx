...
const DocumentsPreviewPage: React.FC = () => {
      ...
      <div className="h-full mr-4">
        <div className="overflow-y-auto custom_scrollbar h-full">
          <div className="h-full w-full max-w-full flex flex-col mb-5">
            <h1 className="text-2xl font-medium mb-5 text-white">Content:</h1>
  ...
  const renderSidebar = () => {
    ...
    return (
      <div className={`border-r border-neutral-800 fixed xl:relative w-full z-50 xl:z-auto
        transition-all duration-300 bg-[#111111] ease-in-out max-w-[200px] md:max-w-[300px] h-[85vh] rounded-md p-5`}>
        <X
          onClick={() => setIsOpen(false)}
          className="text-white mb-4 ml-auto mr-0 cursor-pointer hover:text-neutral-400"
          size={20}
        />

        {documentKeys.length > 0 && (
          <div className="mt-8">
            <p className="text-xs mt-2 text-neutral-300 opacity-70">DOCUMENTS</p>
            <div className="flex flex-col gap-2 mt-6">
              {documentKeys.map((key: string) => (
                <div
                  key={key}
                  onClick={() => updateSelectedDocument(key)}
                  className={`${selectedDocument === key ? "border border-red-600 bg-red-900/10" : ""
                    } flex p-2 rounded-sm gap-2 items-center cursor-pointer`}
                >
                  <img
                    className="h-6 w-6 border border-neutral-800"
                    src={getIconFromFile(key)}
                    alt="Document icon"
                  />
                  <span className="text-sm h-6 text-neutral-300 overflow-hidden">
  ...
  return (
    <div className={`bg-[#111111]/90 min-h-screen flex flex-col w-full`}>
      <OverlayLoader
      ...
      <div className="flex mt-6 gap-4 font-instrument_sans">
          ...
          <div className="fixed left-4 top-1/2 -translate-y-1/2 z-50">
            <ToolTip content="Open Panel">
              <Button
                ...
                className="bg-[#dc2626] text-white p-3 shadow-lg"
              >
        ...
        <div className="bg-[#111111] w-full mx-2 sm:mx-4 h-[calc(100vh-100px)] custom_scrollbar rounded-md overflow-y-auto py-6 pl-6">
        ...
        <div className="fixed bottom-5 right-5">
          <Button
            ...
            className="flex items-center gap-2 px-8 py-6 rounded-sm text-md bg-[#dc2626] hover:bg-[#dc2626]/90"
          >
