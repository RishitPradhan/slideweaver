...
const VERSION_JSON_URL =
  ...
  "https://raw.githubusercontent.com/slideweaver/slideweaver/refs/heads/main/electron/version.json";
...
async function fetchVersionInfo(): Promise<VersionResponse | null> {
  try {
    ...
    const response = await net.fetch(VERSION_JSON_URL, {
      method: "GET",
      headers: { "User-Agent": `Slideweaver/${CURRENT_VERSION}` },
    });
    if (!response.ok) {
...
function injectUpdateBanner(
  ...
  const viewDetailsBtnHtml = hasMessage
    ? '<button id="__slideweaver_view_details_btn__" style="color:#64748b;background:none;border:none;cursor:pointer;font-size:12px;padding:4px 8px;text-decoration:underline;text-underline-offset:2px;">View details</button>'
    : "";

  const script = /* js */ `
    (function () {
      if (document.getElementById('__slideweaver_update_banner__')) return;

      const msgHtml = ${safeMessageJson};

      const banner = document.createElement('div');
      banner.id = '__slideweaver_update_banner__';
      ...
      banner.innerHTML = \`
        <span style="display:flex;align-items:center;gap:8px;flex:1;min-width:0;">
          ...
          <span>
            Slideweaver&nbsp;<strong style="color:#5141e5">${latest}</strong>&nbsp;is available
        ...
        <div style="display:flex;gap:8px;align-items:center;flex-shrink:0;">
          ...
          <button onclick="document.getElementById('__slideweaver_update_banner__').remove();var o=document.getElementById('__slideweaver_update_overlay__');if(o)o.remove();" title="Dismiss" style="background:none;border:none;color:#94a3b8;cursor:pointer;font-size:20px;line-height:1;padding:0 2px;">&times;</button>
        </div>
      \`;
      ...
      if (msgHtml) {
        ...
        overlay.id = '__slideweaver_update_overlay__';
        ...
        overlay.innerHTML = \`
          <div style="background:#fff;border-radius:16px;max-width:420px;width:100%;max-height:80vh;overflow:auto;box-shadow:0 24px 48px rgba(0,0,0,0.15);padding:24px;" onclick="event.stopPropagation()">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
              ...
              <button onclick="document.getElementById('__slideweaver_update_overlay__').style.display='none'" style="background:none;border:none;color:#94a3b8;cursor:pointer;font-size:24px;line-height:1;padding:0;">&times;</button>
            </div>
            <div style="color:#475569;font-size:14px;line-height:1.6;" id="__slideweaver_overlay_content__"></div>
          </div>
        \`;
        document.body.appendChild(overlay);
        document.getElementById('__slideweaver_overlay_content__').innerHTML = msgHtml;
        document.getElementById('__slideweaver_view_details_btn__').onclick = function() {
          document.getElementById('__slideweaver_update_overlay__').style.display = 'flex';
        };
      }
    })();
  `;
